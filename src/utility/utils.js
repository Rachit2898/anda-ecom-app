...
export async function login(body) {
  const token = await getFingerPrintToken();
  console.log("loginTimeDeviceToken", token);
  const credentials = {
    username: body.email,
...
export const getFingerPrintToken = async () => {
  const token = await AsyncStorage.getItem("fingerPrintToken");
  return token;
};
export const getRedirectUrl = async () => {
  const token = await AsyncStorage.getItem("redirectUrl");
