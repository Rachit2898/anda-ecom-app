...
import DeviceVerification from "./screens/DeviceVerification";
...
function AuthenticatedStack() {
  ...
  const { urlUpdated } = useSelector((state) => ({
    ...state.auth,
  }));
  const getUrl = async () => {
    const urlValue = await getRedirectUrl();
    setUrl(urlValue);
  };
  ...
  return (
    <>
      {url === "/verify/device" ? (
        <Stack.Navigator
          initialRouteName={"DeviceVerification"}
