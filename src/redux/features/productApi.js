...
export const notificationDetails = createAsyncThunk(
  ...
  async () => {
    ...
    var url = `${apiUrl}/api/notification/messages/${pushToken}`;
...
export const updateDeviceValidation = createAsyncThunk(
  "urls/updateDeviceValidation",
  async (body) => {
    const token = await getToken();
    ...
    const { apiUrl } = getEnvVars();
    var url = `${apiUrl}/api/customer/verify/device`;
    const response = await fetch(url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const verifyToken = createAsyncThunk(
  ...
  async (body) => {
    const token = await getToken();
    const pushToken = await getFingerPrintToken();
    const credentials = {
      token: body.token,
      tokenReason: "email",
...
export const pushTokenSubmit = createAsyncThunk(
  ...
  async (body) => {
    ...
    console.log("subscribeTimeToken", credentials);
    const { apiUrl } = getEnvVars();
    if (!!token) {
      ...
      const myData = response;
      console.log("subscribeTimeResponse", myData);
...
export const pushTokenRemove = createAsyncThunk(
  ...
  async (body) => {
    ...
    console.log("unSubscribeTimeToken", credentials);
    const { apiUrl } = getEnvVars();
    if (!!token) {
      ...
      const myData = response;
      console.log("unSubscribeTimeResponse", myData);
...
const productSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // verifyDeviceToken
      .addCase(verifyDeviceToken.pending, (state, action) => {
        state.loading = true;
      })
      .addCase(verifyDeviceToken.fulfilled, (state, action) => {
        state.loading = false;
        state.verifyDeviceTokenData = action.payload;
      })
      .addCase(verifyDeviceToken.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      // volumeDiscount
      ...
      .addCase(notificationDetails.pending, (state, action) => {
        state.loading = true;
      })
      .addCase(notificationDetails.fulfilled, (state, action) => {
        state.notificationDetailsData = action.payload;
        state.loading = false;
      })
